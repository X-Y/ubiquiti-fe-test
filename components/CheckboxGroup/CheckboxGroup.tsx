...
function CheckboxGroup({ name, values, onChange }: CheckboxGroupProps) {
  ...
  const checkedKeys = valueEntries
    .filter(([, value]) => !!value)
    .map(([key]) => key);
  ...
  return (
    <>
      <div>
        {valueEntries.map(([key, value]) => (
          <Checkbox
            ...
            checked={groupValue.has(key)}
            onChange={onIndividualChange(key)}
          >
            {key}
          </Checkbox>
        ))}
      ...
      <button
        ...
        disabled={!groupValue.size}
        className="text-red-06 disabled:text-red-03"
      >
        Reset
      </button>
    </>
